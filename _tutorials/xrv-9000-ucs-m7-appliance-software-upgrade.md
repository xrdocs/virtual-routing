...
- Software reimage: this technique is usually used for initial staging or disaster recovery. The procedure is documented in details in [this blog post]({{site.baseurl}}/tutorials/xrv-9000-ucs-m7-appliance-reimage).
- Traditional software upgrade: the appliance is already running a certain IOS XR version which must be upgraded. This second case is documented in this article.

This procedure will strictly focus on the software upgrade aspect, it does not cover:
...
UCS CIMC, BIOS, NIC drivers, RAID controller firmware etc. can be considered as Field Programmable Devices (FPD).  Like any software, those firmwares might need upgrades to add new functionalities, fix bugs, or improve security.  
...
**Important:** Do not download CIMC/BIOS files directly from UCS software download section. Instead, download and use the signed images from the IOS XRv 9000 download section.
{: .notice--danger}
